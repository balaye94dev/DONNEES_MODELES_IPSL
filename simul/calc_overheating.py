...
def read_and_compute_overheating(xls_path="simul/compare.xlsx", threshold=THRESHOLD_C):
    ...
    for name, df in sheets.items():
        ...
        mean_a = exceed_a.max(skipna=True)
        mean_f = exceed_f.max(skipna=True)
...
def plot_overheating(labels, actual_oh, future_oh, out_path=None):
    ...
    bars_a = ax.bar(x - width/2, actual_oh, width, label='Actual', color='C0')
    bars_f = ax.bar(x + width/2, future_oh, width, label='Future', color='C1')

    ax.set_ylabel('Overheating degree (°C)')
    ax.set_title('Indoor overheating Degrees — Actual vs Future (threshold 26°C)')
    ax.set_xticks(x)
    ...
    try:
        ax.bar_label(bars_a, labels=[f"{v:.2f} °C" for v in actual_oh], padding=4, fontsize=7)
        ax.bar_label(bars_f, labels=[f"{v:.2f} °C" for v in future_oh], padding=4, fontsize=7)
    except Exception:
