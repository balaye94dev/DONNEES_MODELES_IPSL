...
df = pd.read_csv("results/statistics/monthly_climatology_historical_1990_2019.csv")
...
for i, zone in enumerate(df["zone"].unique()):
    ...
    ax.plot(z["month"], z["rsds_mean"], marker='o', markersize=3.5, 
            color=colors[i], linestyle='none', markerfacecolor='white',
            markeredgewidth=0.7, zorder=3)

# Add title
ax.set_title('Historical Monthly Mean Irradiation by Region', fontsize=13, fontweight='normal', pad=15)

# Enhance axis labels and formatting
ax.set_xlabel("Month", fontweight='normal', fontsize=11)
ax.set_ylabel("Irradiation (W/m²)", fontweight='normal', fontsize=11)
...
plt.tight_layout()
plt.savefig("results/figures/monthly_irradiation_hist.png", dpi=300, bbox_inches='tight', 
            facecolor='white', edgecolor='none')
print("✅ Monthly irradiation plot saved to results/figures/monthly_irradiation_hist.png")
